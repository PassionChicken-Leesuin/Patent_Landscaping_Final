...
## 도메인 정의
Computer vision is a field focused on enabling machines to interpret and understand visual information from the world, such as images and videos, by performing tasks like object detection, scene segmentation, feature extraction, 3D reconstruction, augmented reality integration, Structure from Motion (SfM), and pose estimation. It involves the development of algorithms and systems that can process, analyze, and derive meaningful insights from visual data, often mimicking human visual perception capabilities.
...
- **C7.** The invention must involve augmented reality techniques that integrate virtual objects with real-world environments using computer vision.
  - 근거: corpus: definition, corpus: task
- **C8.** The invention must involve Structure from Motion (SfM) techniques for estimating 3D structures from 2D image sequences.
  - 근거: corpus: definition, corpus: task
- **C9.** The invention must involve scene segmentation, dividing an image into meaningful segments.
  - 근거: corpus: definition, corpus: task
- **C10.** The invention must involve pose estimation, determining the position and orientation of objects or people.
  - 근거: corpus: definition, corpus: task

## 분석 대상 특허의 범위
The scope of analysis for the computer vision domain includes patents that implement, improve, or provide enabling components or methods specific to computer vision tasks such as object detection, scene segmentation, feature extraction, 3D reconstruction, augmented reality integration, Structure from Motion (SfM), and pose estimation. It also includes specific applications of these technologies, provided they are integral to the computer vision process. Patents that merely use computer vision outputs or vocabulary for unrelated purposes are excluded.
...
- [CONDITIONAL] **Augmented Reality** — In-scope if the AR system uses computer vision techniques to integrate virtual objects with real-world environments; out if it is purely about display technology.
- [IN] **Structure from Motion (SfM)** — SfM is a core computer vision task involving estimating 3D structures from 2D image sequences.

## 제외 기준 (E)

- **E1.** Patents that focus on image processing without specific applications in computer vision tasks are excluded unless they directly support a computer vision task.
...
- **E4.** Medical imaging patents that do not apply computer vision techniques for analysis are excluded.
  - 근거: patent-pool: suspected_boundary_cases
- **E5.** Patents related to augmented reality that do not use computer vision techniques for integrating virtual objects with real-world environments are excluded.
  - 근거: corpus: definition, corpus: task
...
- For patents related to augmented reality, include only if they use computer vision techniques to integrate virtual objects with real-world environments.
- For patents involving Structure from Motion (SfM), include as they are core to computer vision tasks.
- Ensure that augmented reality and Structure from Motion (SfM) are adequately considered during patent analysis, even if underrepresented in the patent pool.
