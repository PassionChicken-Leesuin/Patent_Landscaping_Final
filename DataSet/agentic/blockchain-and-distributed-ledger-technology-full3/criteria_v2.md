...
- **C6.** The invention must use blockchain or distributed ledger technology for secure identity authentication, ensuring it manages identity data securely and immutably.
  - 근거: corpus: definition, https://en.wikipedia.org/wiki/Blockchain
...
- **E2.** Patents that use blockchain or distributed ledger outputs for unrelated purposes without performing the domain's core tasks are excluded.
  - 근거: corpus: boundary_case, patent-pool digest
- **E3.** Patents that involve traditional databases or centralized systems using blockchain terminology without implementing its core functionalities are excluded.
  - 근거: corpus: boundary_case, patent-pool digest
