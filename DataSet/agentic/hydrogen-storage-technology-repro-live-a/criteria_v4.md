...
Hydrogen Storage Technology encompasses inventions that enable the storage of hydrogen in a stable and efficient manner, ensuring safe containment and release of hydrogen under various conditions. This includes methods and materials that optimize the volume and weight of storage systems, enhance the efficiency of hydrogen release, and ensure compatibility with existing hydrogen production and utilization systems. The domain covers mechanical, chemical, and nanomaterial-based storage methods, as well as innovations that specifically improve or enable these technologies.
...
- **C1.** The invention must involve a method or material for storing hydrogen, including mechanical, chemical, or nanomaterial-based approaches.
...
- **C4.** The invention must optimize the volume and weight of hydrogen storage systems to enhance energy density and portability.
  - 관찰 신호(비배타적 단서): lightweight materials, energy density, compact design, portable storage
  - 기술축: A4
  - [user_query/high] query.json: The technology must optimize the volume and weight of hydrogen storage systems.
  - [web/high] https://en.wikipedia.org/wiki/Hydrogen_storage: Hydrogen storage in vehicles requires storing hydrogen in an energy-dense form to provide sufficient driving range.
  - 레거시 출처: https://en.wikipedia.org/wiki/Hydrogen_storage, query.json
- **C5.** The invention must be compatible with existing hydrogen production and utilization systems.
  - 관찰 신호(비배타적 단서): system integration, compatibility, hydrogen production, fuel cells
  - 기술축: A5
  - [user_query/high] query.json: The technology must be compatible with existing hydrogen production and utilization systems.
  - [corpus/high] corpus: hydrogen production methods: Innovative hydrogen production methods using renewable energy sources
  - 레거시 출처: query.json, corpus:case:3
- **C6.** The invention must involve the use of Metal-Organic Frameworks (MOFs) for hydrogen storage, leveraging their high surface area and porosity to enhance storage capacity.
  - 관찰 신호(비배타적 단서): MOFs, high surface area, porous materials, hydrogen uptake
  - 기술축: A7
  - [web/high] https://en.wikipedia.org/wiki/Metal–organic_framework: Metal–organic frameworks (MOFs) are used for hydrogen storage due to their porous nature.
...
The scope of analysis for Hydrogen Storage Technology includes patents that implement, improve, or provide enabling components or methods specific to the storage, release, and safety of hydrogen. This encompasses mechanical, chemical, and nanomaterial-based storage methods, as well as innovations that enhance the efficiency, safety, and compatibility of hydrogen storage systems with existing technologies. Patents that merely use hydrogen storage outputs for unrelated purposes or focus on handling rather than storage are excluded.

## 범위 결정

- [OUT] **hydrogen production methods** — While related to hydrogen technology, production methods do not specifically address the storage, release, or safety of hydrogen storage systems.
- [IN] **hydrogen storage materials** — These materials are directly involved in the storage of hydrogen, aligning with the core purpose of the domain.
- [OUT] **hydrogen purification techniques** — Purification techniques focus on refining hydrogen rather than storing it, which is outside the domain's core tasks.
- [OUT] **fuel cell systems** — Fuel cell systems utilize hydrogen but do not specifically address the storage or release of hydrogen.
- [OUT] **hydrogen recovery systems** — Recovery systems focus on reclaiming hydrogen rather than storing it, which is not a core task of the domain.
- [OUT] **hydrogen generation systems** — Generation systems produce hydrogen but do not specifically address its storage or release.
- [OUT] **Air supply systems for fuel cells** — These systems focus on air management rather than hydrogen storage or generation.
- [CONDITIONAL] **Liquid hydrogen handling technologies** — Liquid hydrogen handling technologies are included if they directly contribute to the safety and efficiency of hydrogen storage solutions.

## 제외 판단 기준 (E)

- **E1.** Patents that focus on hydrogen production methods without addressing storage, release, or safety are excluded.
  - 관찰 신호(비배타적 단서): hydrogen production, electrolyzer, generation
  - [corpus/high] corpus:cluster:1: hydrogen production methods
  - 레거시 출처: corpus:cluster:1
- **E2.** Patents that focus on hydrogen purification techniques without addressing storage, release, or safety are excluded.
  - 관찰 신호(비배타적 단서): purification, refinement, filtering
  - [corpus/high] corpus:cluster:3: hydrogen purification techniques
  - 레거시 출처: corpus:cluster:3
- **E3.** Patents that focus on fuel cell systems without addressing storage, release, or safety are excluded.
  - 관찰 신호(비배타적 단서): fuel cell, electricity generation, power conversion
  - [corpus/high] corpus:cluster:4: fuel cell systems
  - 레거시 출처: corpus:cluster:4
- **E4.** Patents that focus on hydrogen recovery systems without addressing storage, release, or safety are excluded.
  - 관찰 신호(비배타적 단서): recovery, reclamation, industrial applications
  - [corpus/high] corpus:cluster:5: hydrogen recovery systems
  - 레거시 출처: corpus:cluster:5

## 경계 판정 지침

- Air supply systems for fuel cells are out of scope as they focus on air management rather than hydrogen storage or generation.
- Liquid hydrogen handling technologies are out of scope as they relate to handling rather than the storage technology itself.

## HITL이 필요한 범위 질문

- **Q1. Should the domain include nanomaterials specifically for enhancing hydrogen storage, despite their underrepresentation in the patent pool?**
  - 영향: 측정: 풀 표본 20건 중 3건(~15%)의 판정이 넓게/좁게에 따라 갈립니다. Nanomaterials are highlighted in web evidence as significant for hydrogen storage, but are underrepresented in the patent pool, affecting the domain's breadth.
...
- **Q2. Should the domain include metal-organic frameworks (MOFs) for hydrogen storage, given their significance in web evidence but lack of emphasis in the patent pool?**
  - 영향: 측정: 풀 표본 20건 중 2건(~10%)의 판정이 넓게/좁게에 따라 갈립니다. MOFs are significant in web evidence for hydrogen storage but are underrepresented in the patent pool, affecting the domain's scope.
  - 선택지: Include MOFs for hydrogen storage, Exclude MOFs for hydrogen storage
  - 미응답 기본값: Include MOFs for hydrogen storage
