...
- **C7.** The invention must utilize AI or machine learning for perception, decision-making, or control in autonomous vehicles.
  - 근거: corpus: technique, corpus: mismatch_with_web_evidence
...
- [CONDITIONAL] **Cruise Control Systems** — In if the system includes autonomous features like adaptive cruise control that contribute to autonomous navigation; out if it only maintains a set speed without autonomous capabilities.
...
- **E4.** Patents that pertain to unmanned aerial or marine vehicles are excluded unless they are used for autonomous navigation similar to ground vehicles.
  - 근거: corpus: boundary_case
