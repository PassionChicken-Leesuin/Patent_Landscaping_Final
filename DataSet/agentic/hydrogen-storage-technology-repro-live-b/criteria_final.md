...
- **C6.** The invention must utilize advanced materials such as metal-organic frameworks (MOFs) and nanomaterials to enhance hydrogen storage capabilities.
  - 관찰 신호(비배타적 단서): advanced materials, MOFs, nanomaterials, enhanced storage
  - 기술축: A6
  - [web/high] https://en.wikipedia.org/wiki/Metal–organic_framework: Metal–organic frameworks (MOFs) are used for hydrogen storage due to their porous nature.
  - [web/high] https://en.wikipedia.org/wiki/Hydrogen_storage: Nanomaterials can enhance hydrogen storage by improving sorption kinetics and storage capacity.
  - [corpus/high] corpus:mismatch:1: The patent pool lacks emphasis on Metal–Organic Frameworks (MOFs) for hydrogen storage, which are highlighted in web evidence.
  - 레거시 출처: https://en.wikipedia.org/wiki/Metal–organic_framework, https://en.wikipedia.org/wiki/Hydrogen_storage, corpus:mismatch:1
...
- [CONDITIONAL] **Electrochemical reaction apparatus** — In if the apparatus specifically addresses hydrogen storage, supported by evidence from corpus:boundary:2; out if it involves general electrochemical processes.
...
- **Q1. Should the domain include electrochemical reaction apparatuses that do not specifically address hydrogen storage?**
  - 영향: 측정: 풀 표본 20건 중 8건(~40%)의 판정이 넓게/좁게에 따라 갈립니다. Electrochemical processes can be used for both storage and other purposes, affecting domain inclusion.
