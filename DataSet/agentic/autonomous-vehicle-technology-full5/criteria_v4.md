...
- **C3.** The invention must incorporate systems that ensure passenger safety through technologies like forward collision prevention or lane keeping assistance, focusing on collision avoidance and obstacle detection.
  - 근거: corpus: Collision Avoidance Systems, corpus: Lane Keeping and Departure Systems, corpus: Obstacle Detection and Avoidance
...
- [CONDITIONAL] **Cruise Control Systems** — In if the cruise control system contributes to vehicle autonomy by enabling autonomous driving functions; out if it only maintains speed without contributing to autonomy.
...
- **E1.** Patents that focus on driver assistance systems that do not contribute to vehicle autonomy are excluded, unless they are integrated into a broader autonomous system.
...
- Object Detection is in scope if it is used for autonomous navigation or obstacle avoidance, but out of scope if it is used for purposes unrelated to vehicle navigation or safety.
