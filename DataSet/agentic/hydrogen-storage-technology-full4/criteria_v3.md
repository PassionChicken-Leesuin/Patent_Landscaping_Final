...
- **C1.** The invention must describe a specific method or technology that is applied to perform hydrogen storage, such as compression, liquefaction, or chemical storage using metal hydrides, in the title or abstract.
  - 근거: corpus: technique
- **C2.** The invention must indicate a mechanism for retrieving hydrogen from its storage medium for energy applications, as evidenced by terms like 'release', 'extraction', or 'retrieval' in the title or abstract.
  - 근거: corpus: task
- **C3.** The invention must reference safety measures related to hydrogen storage, such as 'leakage prevention', 'explosion risk', or 'flammability control', in the title or abstract.
  - 근거: corpus: task
- **C4.** The invention must include terms related to optimizing hydrogen storage efficiency specifically applied to hydrogen storage processes, such as 'volume efficiency', 'weight efficiency', or 'advanced materials' like MOFs or nanomaterials, in the title or abstract.
  - 근거: corpus: technique
- **C5.** The invention must mention maintaining hydrogen purity during storage, using terms like 'purity', 'contamination prevention', or 'clean storage' in the title or abstract.
  - 근거: corpus: task
...
- [CONDITIONAL] **Hydrogen purification methods** — In if the purification process is integral to the storage system, ensuring hydrogen purity during storage; out if it focuses solely on purification without a direct link to storage.
...
- **E2.** Patents that involve hydrogen utilization in applications like fuel cells or industrial processes without focusing on storage are excluded.
  - 근거: corpus: task
- **E3.** Patents that focus on hydrogen purification methods that are not integral to storage systems are excluded.
  - 근거: corpus: task
