...
- **C2.** The invention must facilitate a consensus mechanism, such as proof-of-work or proof-of-stake, to validate and agree on the state of the ledger among participants.
  - 근거: https://en.wikipedia.org/wiki/Consensus_(computer_science), https://en.wikipedia.org/wiki/Blockchain, https://en.wikipedia.org/wiki/Proof_of_work, https://en.wikipedia.org/wiki/Proof_of_stake
...
- [IN] **Energy transactions using blockchain** — Energy transactions using blockchain can enhance transparency and efficiency, aligning with the domain's core tasks.
- [IN] **Healthcare transaction validation** — Healthcare transaction validation using blockchain ensures secure and tamper-proof processes, aligning with the domain's core tasks.
...
- [OUT] **Photography-related patents** — These patents do not align with core blockchain functionalities and focus on photography.
- [OUT] **Payment processing using sound signals** — These patents focus on payment processing using sound signals, with blockchain as a secondary element.
...
- **E2.** Patents that use blockchain terminology but do not implement or improve blockchain technology are excluded.
  - 근거: corpus: suspected_boundary_cases
- **E3.** Patents that focus on centralized systems or traditional databases using similar terminology are excluded.
  - 근거: corpus: suspected_boundary_cases
...
- For patents like 'A kind of photographs based on block chain really weighs method, device and equipment,' exclude if the focus is on photography rather than blockchain technology.
- For patents like 'Method for Providing Asynchronous Reverse Direction Payment by using Sound Signal Device and Blockchain,' exclude if blockchain is not the primary focus.
